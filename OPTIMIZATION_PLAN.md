...
`"Apple M4 (Metal)"`와 Apple 투사 모델의 현재 기준값으로 쓴다.
...
(1.029e6 cell-evals/s per GB/s).

#### 결과 (M cell-evals/s)

| 칩 | 스펙 | 달성 추정 | 구속 한계 | throughput |
|---|---|---|---|---|
| M1 | 68.2 | 58.0 | memory | 59.7 |
| M2 | 100 | 85.0 | memory | 87.5 |
| M3 | 102.4 | 87.0 | memory | 89.6 |
| **M4** | **120** | **102.0** | **memory** | **105.0 (측정)** |
| M1 Pro | 200 | 116.0 | gpu-width | 119.4 |
| M3 Pro | 153.6 | 130.6 | memory | 134.4 |
| M5 | 153.6 | 130.6 | memory | 134.4 |
| M2 Pro | 200 | 161.5 | gpu-width | 166.2 |
| M4 Pro | 273 | 204.0 | gpu-width | 210.0 |
| M1 Max | 400 | 232.0 | gpu-width | 238.8 |
| M3 Max 30c | 300 | 255.0 | memory | 262.4 |
| M5 Pro | 307 | 260.9 | memory | 268.6 |
| M2 Max | 400 | 323.0 | gpu-width | 332.4 |
| M4 Max 32c | 410 | 326.4 | gpu-width | 335.9 |
| M3 Max 40c | 409.6 | 348.2 | memory | 358.3 |
| M5 Max 32c | 460 | 391.0 | memory | 402.4 |
| M4 Max 40c | 546 | 408.0 | gpu-width | 419.9 |
| M1 Ultra | 800 | 464.1 | gpu-width | 477.6 |
| M5 Max 40c | 614 | 521.9 | memory | 537.1 |
| M2 Ultra | 800 | 578.0 | memory | 594.9 |
| M3 Ultra | 819.3 | 591.9 | memory | 609.2 |
...
M5 Max 40c(537.1)가 M1 Ultra(477.6)를 넘어서면서 세대 역전은 해소됐다.
...
- `bench/apple-vs-gpus.svg` — CUDA 60개 실측 + Apple 측정/투사를 한 축에. Apple 칩이
  어느 CUDA GPU 사이에 놓이는지 바로 보인다. M4는 GTX 1650 mobile과 GTX 970 사이,
  M3 Ultra는 V100-SXM2-32GB 바로 아래, M5 Max 40c는 TITAN RTX와 RTX 3070 Ti 사이다.
...
| 구간 | Apple | PC | PC 배수 |
|---|---|---|---|
| entry | Mac mini M4 $599 → 105 (175/$1k) | RTX 4060 Ti PC $1,299 → 300 (231/$1k) | **2.9×** |
| mid | Mac mini M4 Pro $1,399 → 210 (150/$1k) | RTX 4070 PC $1,399 → 499 (357/$1k) | **2.4×** |
| high | Mac Studio M4 Max $2,499 → 419.9 (168/$1k) | RTX 5080 PC $1,799 → 926 (515/$1k) | **2.2×** |
| top | Mac Studio M3 Ultra $5,299 → 609.2 (115/$1k) | RTX 5090 PC $2,799 → 1917 (685/$1k) | **3.1×** |

**이 워크로드에서는 같은 예산이면 PC가 2.2~3.1배 빠르다.** 달러당 처리량은 PC가 1.3~6.0배 좋고,
Apple 쪽에서 달러당 가장 좋은 건 Mac mini M4(175/$1k)이며 M3 Ultra가 가장 나쁘다(115/$1k).
top 구간은 Mac이 1.9배 더 비싸면서 0.32배 성능이다.
...
| | M cell-evals/s per GB/s |
|---|---|
| 최신 consumer CUDA 중앙값 | **0.990** |
| Apple/Metal 착수 시점 | 0.645 |
| Apple/Metal 현재 | **0.875** |
| **남은 소프트웨어 격차** | **1.13×** (착수 시점 1.53×) |

완제품 $1k당 대역폭은 Mac 155~219 GB/s, PC 222~640 GB/s다. **상위 구간에서 NVIDIA가 달러당
대역폭을 2.4~4.1배 판다.** 최신 consumer CUDA와의 소프트웨어 비는
`0.990 / 0.875 = 1.13×`이므로, 동가격대 실차의 대부분은 하드웨어 대역폭·가격 차이다.

즉 **2.2~3.1× 중 코드 몫은 1.13×뿐이다.** 나머지는 하드웨어 가격이며 코드로 뚫을 수 없다.
...
2.2~3.1배 빠르다. 그 위에서는 최상위 consumer NVIDIA로 **아예 실행되지 않고**, Mac Studio는
...
권고했던 4개가 전부 기각됐다는 사실 자체가 결과다. 남은 소프트웨어 여유는 **1.13×**이고,
...
9절까지의 결론("남은 소프트웨어 여유 1.13×")은 **4.19M 셀 한 지점에서만** 옳았다.
