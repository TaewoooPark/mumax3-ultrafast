...
// results each reduction writes. Each threadgroup owns one slot; the host
// combines the slots in fixed index order so the result is reproducible.
const reduceSlots = 8
...
func Dot(a, b *data.Slice) float32 {
	...
	for c := 0; c < nComp; c++ {
		k_reducedot_async(a.DevPtr(c), b.DevPtr(c), slot(out, c), 0, a.Len(), reducecfg)
	}
	return copybackSum(out, nComp)
}
...
func MaxVecNorm(v *data.Slice) float64 {
	...
	return math.Sqrt(float64(copybackMax(out, 1)))
}
...
func slot(buf unsafe.Pointer, c int) unsafe.Pointer {
	return unsafe.Pointer(uintptr(buf) + uintptr(c*reduceSlots)*cu.SIZEOF_FLOAT32)
}

// return a reduction buffer from a pool, with nComp*reduceSlots partial slots
// initialized to initVal
func reduceBuf(initVal float32, nComp int) unsafe.Pointer {
	if reduceBuffers == nil {
		initReduceBuf()
	}
	buf := <-reduceBuffers
	cu.MemsetD32Async(cu.DevicePtr(uintptr(buf)), math.Float32bits(initVal),
		int64(nComp*reduceSlots), stream0)
	return buf
}

// copybackSum copies the partial results back and adds them in fixed index
// order, then recycles the buffer.
func copybackSum(buf unsafe.Pointer, nComp int) float32 {
	partial := copybackPartials(buf, nComp)
	var result float32
...
func copybackMax(buf unsafe.Pointer, nComp int) float32 {
	partial := copybackPartials(buf, nComp)
	result := partial[0]
...
func copybackPartials(buf unsafe.Pointer, nComp int) []float32 {
	partial := make([]float32, nComp*reduceSlots)
...
func initReduceBuf() {
	...
	for i := 0; i < N; i++ {
		reduceBuffers <- MemAlloc(3 * reduceSlots * cu.SIZEOF_FLOAT32)
	}
}

// launch configuration for reduce kernels. Grid.X must equal reduceSlots: the
// kernels write one partial per threadgroup at index blockIdx.x.
var reducecfg = &config{Grid: cu.Dim3{X: reduceSlots, Y: 1, Z: 1}, Block: cu.Dim3{X: REDUCE_BLOCKSIZE, Y: 1, Z: 1}}
